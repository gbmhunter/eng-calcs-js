...
<table id="lt3745" class="candy-calc" border="4" style="border-collapse: collapse; width: 100%; max-width: 1000px;" >
	<tbody>
		<tr>
			<td class="heading name">Variable Name</td>
			<td class="heading symbol">Variable Symbol</td>
			<td class="heading value">Value</td>
			<td class="heading units">Units</td>
			<td class="heading comments">Comments</td>
		</tr>
		<tr>
			...
			<td class="comments">The supply voltage for the logic. Must be between 3.0 and 5.5V.</td>
		</tr>	
		<tr>
			...
			<td class="comments">This is the maximum voltage that the load will ever see. If driving LEDs, this is equal to the forward voltage of the LED at the maximum current to plan to drive them at. If driving multiple LEDs series, sum the forward voltages. If driving different colours, this is equal to the LED with the highest forward voltage.</td>
		</tr>		
		<tr>
			...
			<td class="comments">This must be equal or higher than \( V_{load} \). It is recommended to be set between 0.8V and 3.0V above \( V_{load} \) for the best current regulation.</td>
		</tr>
		<tr>
			...
			<td class="comments">The is a minimum input voltage allowed to sustain current regulation. It cannot be less than 6V. The \(2.1V\) is the minimum dropout voltage between the \(V_{in}\) and ISN pins.<span class="equation">\( V_{in(min)} = V_{out(max)} + 2.1V\)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">This is the maximum input voltage that will ever be provided to the LT3745. Must be greater or equal to \( V_{in(min)} \), and less or equal to 55V.</td>
		<tr>
		<tr />
		<tr>
			...
			<td class="comments">This resistor along with \(R_{fb2}\) determines the output voltage of the buck converter. This is recommended to be \(10k\Omega\)</td>
		</tr>
		<tr>
			...
			<td class="comments">This resistor along with \(R_{fb1}\) determines the output voltage of the buck converter.<span class="equation">\( R_{fb2} = R_{fb1}*\left(\dfrac{V_{out(max)}}{1.205V} - 1\right) \)</span></td>
		</tr>
		<tr></tr>
		<tr style="border-bottom: 4px solid #000;">
			...
			<td class="comments">This is the maximum output current for all channels, i.e. maximum current going through MOSFET and current sense resistor.</td>
		</tr>
		<tr>
			...
			<td class="comments">The value for the current-sense resistor which will give you the \( I_{out(max)}\) you want.<span class="equation">\( R_{sense} = \dfrac{35mV}{I_{out(max)}}\)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">This is the power that will dissipated through the current-sense resistor at maximum current output. Make sure the resistor is rated to handle this power.<span class="equation">\( P_{Rsense} = I_{out(max)}^2 * R_{sense}\)</span></td>
		</tr>
		<tr></tr>
		<tr>
			...
			<td class="comments">This is the "nominal" current each LED will see. It has to be between 10-50mA. Note that the current for each channel can be individually controlled down to 50% and up to 150% of this nominal current, and then further modulated with PWM from 0 to 100%.</td>
		</tr>
		<tr style="border-bottom: 4px solid #000;">
			...
			<td class="comments">The resistor sets the nominal LED current chosen above.<span class="equation">\( R_{iset} = 2500*(\dfrac{1.205}{I_{led-pin(nom)}})\)</span></td>
		</tr>
		<tr></tr>
		<tr>
			...
			<td class="comments">This is the forward voltage drop across the buck diode at the operating current. This value can be found in the diodes datasheet. Should be around 0.5V.</td>
		</tr>
		<tr>
			...
			<td class="comments">The minimum duty cycle of the buck converter. This limits the maximum switching frequency.<span class="equation">\( D_{min} = \dfrac{V_{out(max)} + V_{d,f}}{V_{in(max)} + V_{d,f}}\)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">The maximum duty cycle of the buck converter. This limits the maximum switching frequency.<span class="equation">\( D_{max} = \dfrac{V_{out(max)} + V_{d,f}}{V_{in(min)} + V_{d,f}}\)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">This is the minimum switch-on time of the MOSFET. Sometimes called the turn-on delay time ( \( t_{d(on)} \) ). Check the MOSFET's datasheet for this value. Should be greater than 1ns and less than 500ns.</td>
		</tr>
		<tr>
			...
			<td class="comments">This is the minimum switch-off time of the MOSFET. Sometimes called the turn-off delay time ( \( t_{d(off)} \) ). Check the MOSFET's datasheet for this value. Should be greater than 1ns and less than 500ns.</td>
		</tr>
		<tr>
			...
			<td class="comments">This is the maximum switching frequency you could use.<span class="equation">\( f_{sw(max)} = min( \dfrac{D_{min}}{t_{on,min}}, \dfrac{1 - D_{max}}{t_{off(min)}})\)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">This is the switching frequency you want to use, set by the resistor \( R_T \). It has to be between 100kHz and 1MHz, and also less than \(f_{sw(max)}\).</td>
		</tr>
		<tr>
			...
			<td class="comments">This is the switching frequency which would give unity voltage gain between input and output.<span class="equation">\( f_{ugf} = \dfrac{f_{sw(act)}}{10}\)</span></td>
		</tr>
		<tr style="border-bottom: 4px solid #000;">
			...
			<td class="comments">This is the resistance required to set the frequency at \( f_{sw(act)} \) chosen above. Equation was worked out by fitting a power equation to the frequency-resistance values given in the datahseet. This equation fits the data well, with a regression coefficient of \( r^2 = 0.9994 \) within the valid range.<span class="equation">\( R_T = \dfrac{2.25167e^{11}}{f_{sw(act)^{1.114}}} \)</span></td>
		</tr>
		<!-- Temperature stuff -->
		<tr>
		...
		<tr style="border-bottom: 4px solid #000;">
			...
			<td class="comments">The resistance required to be connected between pin \( T_{SET} \) and ground on the LT3745 to limit the junction temperature to \( T_{J(max)} \).<span class="equation">\( R_{TSET} = \frac{1.72mV*(T_J + 273.15)*R_{ISET}}{1.205V} \)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">The output capacitance smooths the output voltage, and also stores energy to satisfy load transients.<span class="equation">\( C_{out(min)} = \small \begin{split} max( \dfrac{0.25}{R_{sense}*f_{ugf}}, \\ \dfrac{1.5}{V_{buck,out}*R_{sense}*f_{ugf}}) \end{split} \)</span></td>
		</tr>
		<tr>
			...
			<td class="comments">This is the maximum desired ripple current through the inductor. A value between 10-50% is recommended.</td>
		</tr>
		<tr>
			...
			<td class="comments">This is the minimum inductance required to satisfy the inductor ripple current specified above. If this inductance is too large, you could consider increasing the ripple current, or increasing the switching frequency.<span class="equation">\( L_{min} = \small \begin{split} \dfrac{V_{buck,out} + V_{d,f}}{ V_{in(max) + V_{d,f}}} * \\ \dfrac{ V_{in(max)} - V_{buck,out} }{ f_{sw(act)}*I_{L(delta)} } \end{split} \)</span></td>
		</tr>
		<tr></tr>
		<tr>
			...
			<td class="comments">The desired maximum input voltage ripple. A value around 100mV is normal.</td>
		</tr>
		<tr>
			...
			<td class="comments">This is the minimum input capacitance required to satisfy the desired input voltage ripple chosen above.<span class="equation">\( V_{in, ripple} = \dfrac{D_{max}*I_{out(max)}}{V_{in,ripple}*f_{sw(act)}} \)</span></td>
		</tr>
	</tbody>
</table>
<!--</div>-->
